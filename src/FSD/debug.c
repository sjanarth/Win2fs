 ...
 __U32 Ext2DbgPrintCall (__U32 flag, PDEVICE_OBJECT DeviceObject, PIRP Irp)
 {
    ...
    switch (IoStackLocation->MajorFunction)
    {
		...
		case IRP_MJ_QUERY_VOLUME_INFORMATION:
			...
			if (FsInformationClass <= FileFsMaximumInformation)
			{
				Ext2DbgPrint(flag, "%s %s %s",
			                gsIRPMajorStrings[IoStackLocation->MajorFunction],
							FileName,
							gsFsInfoClassStrings[FsInformationClass]
						);
			}
			else
			{
				Ext2DbgPrint(flag, "%s %s Unknown FsInformationClass %u",
			                gsIRPMajorStrings[IoStackLocation->MajorFunction],
							FileName,
							FsInformationClass
						);
		...
		case IRP_MJ_DIRECTORY_CONTROL:
			...
			if (IoStackLocation->MinorFunction & IRP_MN_QUERY_DIRECTORY)
			{
				...
				if (FileInformationClass <= FileMaximumInformation)
				{
					...
					{
	#ifndef _GNU_NTIFS_
						Ext2DbgPrint(flag, "FileName: %.*S FileIndex: %x %s%s%s",
					...
					{
	#ifndef _GNU_NTIFS_
						Ext2DbgPrint(flag, "FileName: FileIndex: %#x %s%s%s",
							IoStackLocation->Parameters.QueryDirectory.FileIndex,
							(IoStackLocation->Flags & SL_RESTART_SCAN ? "SL_RESTART_SCAN " : ""),
							(IoStackLocation->Flags & SL_RETURN_SINGLE_ENTRY ? "SL_RETURN_SINGLE_ENTRY " : ""),
							(IoStackLocation->Flags & SL_INDEX_SPECIFIED ? "SL_INDEX_SPECIFIED " : "")
	...
	#else
						Ext2DbgPrint(flag, "FileName: FileIndex: %#x %s%s%s",
							((PEXTENDED_IO_STACK_LOCATION) IoStackLocation)->Parameters.QueryDirectory.FileIndex,
							(IoStackLocation->Flags & SL_RESTART_SCAN ? "SL_RESTART_SCAN " : ""),
							(IoStackLocation->Flags & SL_RETURN_SINGLE_ENTRY ? "SL_RETURN_SINGLE_ENTRY " : ""),
							(IoStackLocation->Flags & SL_INDEX_SPECIFIED ? "SL_INDEX_SPECIFIED " : "")
						);
			...
			{
				Ext2DbgPrint(flag, "%s %s IRP_MN_NOTIFY_CHANGE_DIRECTORY",
					Ext2DbgGetCurrentProcessName(),
					gsIRPMajorStrings[IoStackLocation->MajorFunction],
					FileName
				);
			...
			{
				Ext2DbgPrint(flag, "%s %s Unknown minor function %#x",
					Ext2DbgGetCurrentProcessName(),
					gsIRPMajorStrings[IoStackLocation->MajorFunction],
					FileName,
		...
		case IRP_MJ_FILE_SYSTEM_CONTROL:
			...
			{
	#ifndef _GNU_NTIFS_
				Ext2DbgPrint(flag, "%s %s IRP_MN_USER_FS_REQUEST FsControlCode: %#x",
					gsIRPMajorStrings[IoStackLocation->MajorFunction],
					FileName,
			...
			{
				Ext2DbgPrint(flag, "%s %s IRP_MN_MOUNT_VOLUME DeviceObject: %#x",
					gsIRPMajorStrings[IoStackLocation->MajorFunction],
					FileName,
			...
			{
				Ext2DbgPrint(flag, "%s %s IRP_MN_VERIFY_VOLUME DeviceObject: %#x",
					gsIRPMajorStrings[IoStackLocation->MajorFunction],
					FileName,
			...
			{
				Ext2DbgPrint(flag, "%s %s IRP_MN_LOAD_FILE_SYSTEM",
					gsIRPMajorStrings[IoStackLocation->MajorFunction],
					FileName
				);
			}
			...
			{
				Ext2DbgPrint(flag, "%s %s IRP_MN_KERNEL_CALL",
					gsIRPMajorStrings[IoStackLocation->MajorFunction],
					FileName
				);
			}
		...
		case IRP_MJ_DEVICE_CONTROL:
			...
			Ext2DbgPrint(flag, "%s %s IoControlCode: %#x",
				gsIRPMajorStrings[IoStackLocation->MajorFunction],
				FileName,
		...
		case IRP_MJ_LOCK_CONTROL:
			...
			{
	#ifndef _GNU_NTIFS_
				Ext2DbgPrint(flag, "%s %s IRP_MN_LOCK Offset: %I64u Length: %I64u Key: %u %s%s",
					gsIRPMajorStrings[IoStackLocation->MajorFunction],
					FileName,
			...
			{
	#ifndef _GNU_NTIFS_
				Ext2DbgPrint(flag, "%s %s IRP_MN_UNLOCK_SINGLE Offset: %I64u Length: %I64u Key: %u",
					gsIRPMajorStrings[IoStackLocation->MajorFunction],
					FileName,
					IoStackLocation->Parameters.LockControl.ByteOffset.QuadPart,
					IoStackLocation->Parameters.LockControl.Length->QuadPart,
			...
			{
				Ext2DbgPrint(flag, "%s %s IRP_MN_UNLOCK_ALL",
					gsIRPMajorStrings[IoStackLocation->MajorFunction],
					FileName
				);
			}
			...
			{
	#ifndef _GNU_NTIFS_
				Ext2DbgPrint(flag, "%s %s IRP_MN_UNLOCK_ALL_BY_KEY Key: %u",
					gsIRPMajorStrings[IoStackLocation->MajorFunction],
					FileName,
					IoStackLocation->Parameters.LockControl.Key
				);
 ...
 __U32 Ext2DbgPrintComplete (PIRP Irp)
 {
    ...
    if (Irp)
	{
		if (Irp->IoStatus.Status != STATUS_SUCCESS)
		{
			...
			nRet = Ext2DbgPrint(D_ALWAYS, "%s Completed, Status: %s (%#x).",
						gsIRPMajorStrings[IoStackLocation->MajorFunction],
 ...
 VOID Ext2DbgCleanup ()
 {
	 ...
	 if (gsDebugOutFile != NULL)	
	 {
		 ZwClose (gsDebugOutFile);
